import type {
  RequestAnimeFilterJson, RequestAnimeJson, RequestAnimeListEntryFilterJson, RequestAnimeListEntryJson,
  RequestAnimeSearchJson, RequestChangePasswordJson, RequestGenreGetByNameJson, RequestLoginJson, RequestNewTokenJson,
  RequestRegisterGenreJson, RequestRegisterStudioJson, RequestRegisterUserJson, RequestStudioGetByNameJson,
  RequestUpdateAnimeListEntryJson, RequestUpdateGenreJson, RequestUpdateImageFormData, RequestUpdateStudioJson,
  RequestUpdateUserJson, ResponseAnimeJson, ResponseAnimeListEntryJson, ResponseAnimeListsJson, ResponseAnimesJson,
  ResponseChangePasswordJson, ResponseErrorJson, ResponseGenreJson, ResponseGenresJson, ResponseRegisteredAnimeJson,
  ResponseRegisteredGenreJson, ResponseRegisteredStudioJson, ResponseRegisteredUserJson, ResponseStudioJson,
  ResponseStudiosJson, ResponseTokensJson, ResponseUpdateImageJson, ResponseUserProfileJson
...
const q = (params: object) => {
  ...
  Object.entries(params as Record<string, unknown>).forEach(([k, v]) => {
    ...
    if (Array.isArray(v)) { v.forEach((x) => s.append(k, String(x))); return; }
    s.set(k, String(v));
  });
...
export const api = {
  ...
  updateUserImage: (token: string, data: RequestUpdateImageFormData) => {
    const form = new FormData(); if (data.image) form.append('image', data.image);
  ...
  searchAnime: (data: RequestAnimeSearchJson) => request<ResponseAnimesJson>(`/anime/search${q(data)}`),
  filterAnime: (data: RequestAnimeFilterJson) => request<ResponseAnimesJson>(`/anime/filter${q(data)}`),
  ...
  listAnimeByUserId: (userId: string, data: RequestAnimeListEntryFilterJson) => request<ResponseAnimeListsJson>(`/animelist/list/${userId}${q(data)}`),
  listMyAnime: (token: string, data: RequestAnimeListEntryFilterJson) => request<ResponseAnimeListsJson>(`/animelist/me/list${q(data)}`, {}, token),
